...
static char *new_word(t_shell *sh, char *temp)
{
	...
	while (temp[sh->i] != '\0')
	{
		...
		{
			if (temp[sh->i] == '\'' || temp[sh->i] == '\\') // este ciclo deve estar errado se existir algo do lado esq da.
			{
				if (temp[sh->i] == '\'')
					pelicas = 1;
				if (sh->i - start > 0) // acrescentado
					left2 = ft_substr(temp, start, sh->i - start);
				sh->i++;
				start = sh->i;
		...
		{
			start = sh->i;
			sh->i++; // verificar "fghj'$USER'"
			while (temp[sh->i] != ' ' && temp[sh->i] != '$' && temp[sh->i] != '\"' && temp[sh->i] != '\0' && temp[sh->i] != '\'' && temp[sh->i] != '\\') 
				sh->i++;
