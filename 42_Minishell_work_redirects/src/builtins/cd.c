...
/*   Updated: 2023/06/11 18:01:20 by nmoreira         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
    cmds->cmd_line[1] = get_env("OLDPWD", shell->envp);
    ...
    if (chdir(cmds->cmd_line[1]) != 0)
    {
        free(cmds->oldpwd);
        printf("minishell: cd: ");
		printf("%s", get_env("OLDPWD", shell->envp));
...
static int get_cd_home(t_shell *shell)
{
    char *go_home;

    go_home = get_env("HOME", shell->envp);
    if (!go_home)
    {
...
static int built_cd_2(t_cmds *cmds, t_shell *shell)
{
    cmds->oldpwd = getcwd(NULL, 0);
